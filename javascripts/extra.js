...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let doxybook_reference_link = Array.from(document.querySelectorAll('.md-typeset a[href*="/doxybook/reference/"]'));
    for (const link of cpp_reference_links.concat(open_std_links).concat(doxybook_reference_link)) {
        ...
        let is_code_reference = link.innerHTML.startsWith('std::') || link.href.includes('/doxybook/reference/')
    ...
    for (const anchor of brokenAnchors) {
        for (const link of doxybook_reference_link) {
